...
public abstract class R1ProductionFixture extends R1HttpFixture {
    ...
    protected Deployment deployment(Path directory)throws Exception {
        ...
        database(api,"ols.api.database",true);api.put("ols.api.semantic-baseline","MVP-2026-09-08.1");api.put("ols.api.node","PRODUCTION_API_IT");api.put("ols.api.cursor-key",encoded(0x65));
        ...
        String keys="ols.api.tenant-keys["+seed.tenant()+"].";api.put(keys+"encryption",encoded(0x51));
        ...
        var worker=new TreeMap<String,Object>();worker.putAll(Map.of("ols.runtime-role","worker","spring.main.banner-mode","off","logging.level.root","OFF","ols.worker.semantic-baseline","MVP-2026-09-08.1","ols.worker.node","PRODUCTION_WORKER_IT"));database(worker,"ols.worker.database",false);
