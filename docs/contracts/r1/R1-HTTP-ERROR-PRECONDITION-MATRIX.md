...
Contract ID: R1-HTTP-V1.5

Delegated-context authority: [ADR-0015](../../adr/ADR-0015-task9-delegated-context.md); Identity V1.1 owns exact candidate/SELF rules. X-On-Behalf-Appointment-Id is an optional single UUID authentication selector and requires explicit paired X-Appointment-Id. Missing means own identity, never implicit delegation. Empty, repeated, comma-list, non-UUID or unpaired selector fails400 VALIDATION_FAILED before slot admission; report safe field names, never values. Well-formed unknown, cross-Tenant, unrelated or inactive/expired choice uniformly fails403 NOT_AUTHORIZED without existence disclosure or own-identity fallback. Invalid credential401 and unavailable IdP/database503 retain existing safe Problem, retryPolicy and unknown-commit semantics. No new-key permission is introduced.

All32 public Bearer operations declare it: original11 public business operations and getSessionContext accept valid HUMAN delegated context; all20 Identity management operations reject any supplied selector. SERVICE rejects it. Internal mTLS does not consume it or change its static Actor. Trusted-Origin CORS allows only the exact additional header, never wildcard. Business request bodies are unchanged. getCurrentWorkCard now also declares the existing400 VALIDATION_FAILED response for malformed authentication selectors. SELF stays own-Principal authenticated identity with no-store BODY/Audit-before-disclosure, maximum101 exact disclosedSources and null on-behalf Audit columns; complete business Actor audit and current authorization still apply to cards/commands/receipts.
...
|---|---|---|---|---|---|---|---|---|
| getSessionContext | GET | /api/v1/session/context | AUTHENTICATED_IDENTITY | NONE | NONE | OWN_PRINCIPAL | 200 | VALIDATION_FAILED,UNAUTHENTICATED,NOT_AUTHORIZED,APPOINTMENT_INACTIVE,NOT_FOUND,RATE_LIMITED,INTERNAL_ERROR,SERVICE_UNAVAILABLE |
| listIdentityProviderUsers | GET | /api/v1/admin/identity/provider-users | ACTOR_CONTEXT | NONE | NONE | IDENTITY_ROOT | 200 | VALIDATION_FAILED,UNAUTHENTICATED,NOT_AUTHORIZED,APPOINTMENT_INACTIVE,NOT_FOUND,RATE_LIMITED,INTERNAL_ERROR,SERVICE_UNAVAILABLE |
| getIdentityAdminOptions | GET | /api/v1/admin/identity/options | ACTOR_CONTEXT | NONE | NONE | IDENTITY_SCOPED | 200 | VALIDATION_FAILED,UNAUTHENTICATED,NOT_AUTHORIZED,APPOINTMENT_INACTIVE,NOT_FOUND,RATE_LIMITED,INTERNAL_ERROR,SERVICE_UNAVAILABLE |
| listIdentityPrincipals | GET | /api/v1/admin/identity/principals | ACTOR_CONTEXT | NONE | NONE | IDENTITY_ROOT | 200 | VALIDATION_FAILED,UNAUTHENTICATED,NOT_AUTHORIZED,APPOINTMENT_INACTIVE,NOT_FOUND,RATE_LIMITED,INTERNAL_ERROR,SERVICE_UNAVAILABLE |
| createIdentityPrincipal | POST | /api/v1/admin/identity/principals | ACTOR_CONTEXT | REQUIRED | NONE | IDENTITY_ROOT | 201 | VALIDATION_FAILED,UNAUTHENTICATED,NOT_AUTHORIZED,APPOINTMENT_INACTIVE,NOT_FOUND,RATE_LIMITED,INTERNAL_ERROR,SERVICE_UNAVAILABLE,IDEMPOTENCY_KEY_REQUIRED,IDEMPOTENCY_KEY_INVALID,COMMAND_PAYLOAD_CONFLICT,IDENTITY_BINDING_CONFLICT,IDENTITY_STATE_CONFLICT,IDENTITY_SELF_LOCKOUT,IDENTITY_LAST_ADMIN,IDENTITY_ORGANIZATION_DEPENDENCY,IDENTITY_RESPONSIBILITY_DEPENDENCY |
| renameIdentityPrincipal | PATCH | /api/v1/admin/identity/principals/{id}/display-name | ACTOR_CONTEXT | REQUIRED | IDENTITY_ETAG | IDENTITY_ROOT | 200 | VALIDATION_FAILED,UNAUTHENTICATED,NOT_AUTHORIZED,APPOINTMENT_INACTIVE,NOT_FOUND,RATE_LIMITED,INTERNAL_ERROR,SERVICE_UNAVAILABLE,IDEMPOTENCY_KEY_REQUIRED,IDEMPOTENCY_KEY_INVALID,COMMAND_PAYLOAD_CONFLICT,IDENTITY_BINDING_CONFLICT,IDENTITY_STATE_CONFLICT,IDENTITY_SELF_LOCKOUT,IDENTITY_LAST_ADMIN,IDENTITY_ORGANIZATION_DEPENDENCY,IDENTITY_RESPONSIBILITY_DEPENDENCY,STALE_IDENTITY,IDENTITY_PRECONDITION_REQUIRED |
| suspendIdentityPrincipal | POST | /api/v1/admin/identity/principals/{id}/suspend | ACTOR_CONTEXT | REQUIRED | IDENTITY_ETAG | IDENTITY_ROOT | 200 | VALIDATION_FAILED,UNAUTHENTICATED,NOT_AUTHORIZED,APPOINTMENT_INACTIVE,NOT_FOUND,RATE_LIMITED,INTERNAL_ERROR,SERVICE_UNAVAILABLE,IDEMPOTENCY_KEY_REQUIRED,IDEMPOTENCY_KEY_INVALID,COMMAND_PAYLOAD_CONFLICT,IDENTITY_BINDING_CONFLICT,IDENTITY_STATE_CONFLICT,IDENTITY_SELF_LOCKOUT,IDENTITY_LAST_ADMIN,IDENTITY_ORGANIZATION_DEPENDENCY,IDENTITY_RESPONSIBILITY_DEPENDENCY,STALE_IDENTITY,IDENTITY_PRECONDITION_REQUIRED |
| resumeIdentityPrincipal | POST | /api/v1/admin/identity/principals/{id}/resume | ACTOR_CONTEXT | REQUIRED | IDENTITY_ETAG | IDENTITY_ROOT | 200 | VALIDATION_FAILED,UNAUTHENTICATED,NOT_AUTHORIZED,APPOINTMENT_INACTIVE,NOT_FOUND,RATE_LIMITED,INTERNAL_ERROR,SERVICE_UNAVAILABLE,IDEMPOTENCY_KEY_REQUIRED,IDEMPOTENCY_KEY_INVALID,COMMAND_PAYLOAD_CONFLICT,IDENTITY_BINDING_CONFLICT,IDENTITY_STATE_CONFLICT,IDENTITY_SELF_LOCKOUT,IDENTITY_LAST_ADMIN,IDENTITY_ORGANIZATION_DEPENDENCY,IDENTITY_RESPONSIBILITY_DEPENDENCY,STALE_IDENTITY,IDENTITY_PRECONDITION_REQUIRED |
| disableIdentityPrincipal | POST | /api/v1/admin/identity/principals/{id}/disable | ACTOR_CONTEXT | REQUIRED | IDENTITY_ETAG | IDENTITY_ROOT | 200 | VALIDATION_FAILED,UNAUTHENTICATED,NOT_AUTHORIZED,APPOINTMENT_INACTIVE,NOT_FOUND,RATE_LIMITED,INTERNAL_ERROR,SERVICE_UNAVAILABLE,IDEMPOTENCY_KEY_REQUIRED,IDEMPOTENCY_KEY_INVALID,COMMAND_PAYLOAD_CONFLICT,IDENTITY_BINDING_CONFLICT,IDENTITY_STATE_CONFLICT,IDENTITY_SELF_LOCKOUT,IDENTITY_LAST_ADMIN,IDENTITY_ORGANIZATION_DEPENDENCY,IDENTITY_RESPONSIBILITY_DEPENDENCY,STALE_IDENTITY,IDENTITY_PRECONDITION_REQUIRED |
| listOrganizationUnits | GET | /api/v1/admin/identity/organizations | ACTOR_CONTEXT | NONE | NONE | IDENTITY_SCOPED | 200 | VALIDATION_FAILED,UNAUTHENTICATED,NOT_AUTHORIZED,APPOINTMENT_INACTIVE,NOT_FOUND,RATE_LIMITED,INTERNAL_ERROR,SERVICE_UNAVAILABLE |
| createOrganizationUnit | POST | /api/v1/admin/identity/organizations | ACTOR_CONTEXT | REQUIRED | NONE | IDENTITY_SCOPED | 201 | VALIDATION_FAILED,UNAUTHENTICATED,NOT_AUTHORIZED,APPOINTMENT_INACTIVE,NOT_FOUND,RATE_LIMITED,INTERNAL_ERROR,SERVICE_UNAVAILABLE,IDEMPOTENCY_KEY_REQUIRED,IDEMPOTENCY_KEY_INVALID,COMMAND_PAYLOAD_CONFLICT,IDENTITY_BINDING_CONFLICT,IDENTITY_STATE_CONFLICT,IDENTITY_SELF_LOCKOUT,IDENTITY_LAST_ADMIN,IDENTITY_ORGANIZATION_DEPENDENCY,IDENTITY_RESPONSIBILITY_DEPENDENCY |
| renameOrganizationUnit | PATCH | /api/v1/admin/identity/organizations/{id}/display-name | ACTOR_CONTEXT | REQUIRED | IDENTITY_ETAG | IDENTITY_SCOPED | 200 | VALIDATION_FAILED,UNAUTHENTICATED,NOT_AUTHORIZED,APPOINTMENT_INACTIVE,NOT_FOUND,RATE_LIMITED,INTERNAL_ERROR,SERVICE_UNAVAILABLE,IDEMPOTENCY_KEY_REQUIRED,IDEMPOTENCY_KEY_INVALID,COMMAND_PAYLOAD_CONFLICT,IDENTITY_BINDING_CONFLICT,IDENTITY_STATE_CONFLICT,IDENTITY_SELF_LOCKOUT,IDENTITY_LAST_ADMIN,IDENTITY_ORGANIZATION_DEPENDENCY,IDENTITY_RESPONSIBILITY_DEPENDENCY,STALE_IDENTITY,IDENTITY_PRECONDITION_REQUIRED |
| closeOrganizationUnit | POST | /api/v1/admin/identity/organizations/{id}/close | ACTOR_CONTEXT | REQUIRED | IDENTITY_ETAG | IDENTITY_SCOPED | 200 | VALIDATION_FAILED,UNAUTHENTICATED,NOT_AUTHORIZED,APPOINTMENT_INACTIVE,NOT_FOUND,RATE_LIMITED,INTERNAL_ERROR,SERVICE_UNAVAILABLE,IDEMPOTENCY_KEY_REQUIRED,IDEMPOTENCY_KEY_INVALID,COMMAND_PAYLOAD_CONFLICT,IDENTITY_BINDING_CONFLICT,IDENTITY_STATE_CONFLICT,IDENTITY_SELF_LOCKOUT,IDENTITY_LAST_ADMIN,IDENTITY_ORGANIZATION_DEPENDENCY,IDENTITY_RESPONSIBILITY_DEPENDENCY,STALE_IDENTITY,IDENTITY_PRECONDITION_REQUIRED |
| listAppointments | GET | /api/v1/admin/identity/appointments | ACTOR_CONTEXT | NONE | NONE | IDENTITY_SCOPED | 200 | VALIDATION_FAILED,UNAUTHENTICATED,NOT_AUTHORIZED,APPOINTMENT_INACTIVE,NOT_FOUND,RATE_LIMITED,INTERNAL_ERROR,SERVICE_UNAVAILABLE |
| createAppointment | POST | /api/v1/admin/identity/appointments | ACTOR_CONTEXT | REQUIRED | NONE | IDENTITY_ROOT | 201 | VALIDATION_FAILED,UNAUTHENTICATED,NOT_AUTHORIZED,APPOINTMENT_INACTIVE,NOT_FOUND,RATE_LIMITED,INTERNAL_ERROR,SERVICE_UNAVAILABLE,IDEMPOTENCY_KEY_REQUIRED,IDEMPOTENCY_KEY_INVALID,COMMAND_PAYLOAD_CONFLICT,IDENTITY_BINDING_CONFLICT,IDENTITY_STATE_CONFLICT,IDENTITY_SELF_LOCKOUT,IDENTITY_LAST_ADMIN,IDENTITY_ORGANIZATION_DEPENDENCY,IDENTITY_RESPONSIBILITY_DEPENDENCY |
| suspendAppointment | POST | /api/v1/admin/identity/appointments/{id}/suspend | ACTOR_CONTEXT | REQUIRED | IDENTITY_ETAG | IDENTITY_SCOPED | 200 | VALIDATION_FAILED,UNAUTHENTICATED,NOT_AUTHORIZED,APPOINTMENT_INACTIVE,NOT_FOUND,RATE_LIMITED,INTERNAL_ERROR,SERVICE_UNAVAILABLE,IDEMPOTENCY_KEY_REQUIRED,IDEMPOTENCY_KEY_INVALID,COMMAND_PAYLOAD_CONFLICT,IDENTITY_BINDING_CONFLICT,IDENTITY_STATE_CONFLICT,IDENTITY_SELF_LOCKOUT,IDENTITY_LAST_ADMIN,IDENTITY_ORGANIZATION_DEPENDENCY,IDENTITY_RESPONSIBILITY_DEPENDENCY,STALE_IDENTITY,IDENTITY_PRECONDITION_REQUIRED |
| resumeAppointment | POST | /api/v1/admin/identity/appointments/{id}/resume | ACTOR_CONTEXT | REQUIRED | IDENTITY_ETAG | IDENTITY_SCOPED | 200 | VALIDATION_FAILED,UNAUTHENTICATED,NOT_AUTHORIZED,APPOINTMENT_INACTIVE,NOT_FOUND,RATE_LIMITED,INTERNAL_ERROR,SERVICE_UNAVAILABLE,IDEMPOTENCY_KEY_REQUIRED,IDEMPOTENCY_KEY_INVALID,COMMAND_PAYLOAD_CONFLICT,IDENTITY_BINDING_CONFLICT,IDENTITY_STATE_CONFLICT,IDENTITY_SELF_LOCKOUT,IDENTITY_LAST_ADMIN,IDENTITY_ORGANIZATION_DEPENDENCY,IDENTITY_RESPONSIBILITY_DEPENDENCY,STALE_IDENTITY,IDENTITY_PRECONDITION_REQUIRED |
| endAppointment | POST | /api/v1/admin/identity/appointments/{id}/end | ACTOR_CONTEXT | REQUIRED | IDENTITY_ETAG | IDENTITY_SCOPED | 200 | VALIDATION_FAILED,UNAUTHENTICATED,NOT_AUTHORIZED,APPOINTMENT_INACTIVE,NOT_FOUND,RATE_LIMITED,INTERNAL_ERROR,SERVICE_UNAVAILABLE,IDEMPOTENCY_KEY_REQUIRED,IDEMPOTENCY_KEY_INVALID,COMMAND_PAYLOAD_CONFLICT,IDENTITY_BINDING_CONFLICT,IDENTITY_STATE_CONFLICT,IDENTITY_SELF_LOCKOUT,IDENTITY_LAST_ADMIN,IDENTITY_ORGANIZATION_DEPENDENCY,IDENTITY_RESPONSIBILITY_DEPENDENCY,STALE_IDENTITY,IDENTITY_PRECONDITION_REQUIRED |
| listAuthorityGrants | GET | /api/v1/admin/identity/authority-grants | ACTOR_CONTEXT | NONE | NONE | IDENTITY_SCOPED | 200 | VALIDATION_FAILED,UNAUTHENTICATED,NOT_AUTHORIZED,APPOINTMENT_INACTIVE,NOT_FOUND,RATE_LIMITED,INTERNAL_ERROR,SERVICE_UNAVAILABLE |
| createAuthorityGrant | POST | /api/v1/admin/identity/authority-grants | ACTOR_CONTEXT | REQUIRED | NONE | IDENTITY_SCOPED | 201 | VALIDATION_FAILED,UNAUTHENTICATED,NOT_AUTHORIZED,APPOINTMENT_INACTIVE,NOT_FOUND,RATE_LIMITED,INTERNAL_ERROR,SERVICE_UNAVAILABLE,IDEMPOTENCY_KEY_REQUIRED,IDEMPOTENCY_KEY_INVALID,COMMAND_PAYLOAD_CONFLICT,IDENTITY_BINDING_CONFLICT,IDENTITY_STATE_CONFLICT,IDENTITY_SELF_LOCKOUT,IDENTITY_LAST_ADMIN,IDENTITY_ORGANIZATION_DEPENDENCY,IDENTITY_RESPONSIBILITY_DEPENDENCY |
| revokeAuthorityGrant | POST | /api/v1/admin/identity/authority-grants/{id}/revoke | ACTOR_CONTEXT | REQUIRED | IDENTITY_ETAG | IDENTITY_SCOPED | 200 | VALIDATION_FAILED,UNAUTHENTICATED,NOT_AUTHORIZED,APPOINTMENT_INACTIVE,NOT_FOUND,RATE_LIMITED,INTERNAL_ERROR,SERVICE_UNAVAILABLE,IDEMPOTENCY_KEY_REQUIRED,IDEMPOTENCY_KEY_INVALID,COMMAND_PAYLOAD_CONFLICT,IDENTITY_BINDING_CONFLICT,IDENTITY_STATE_CONFLICT,IDENTITY_SELF_LOCKOUT,IDENTITY_LAST_ADMIN,IDENTITY_ORGANIZATION_DEPENDENCY,IDENTITY_RESPONSIBILITY_DEPENDENCY,STALE_IDENTITY,IDENTITY_PRECONDITION_REQUIRED |
...
| getCurrentWorkCard | GET | /api/v1/workcards/current | ACTOR_CONTEXT | NONE | OPTIONAL_WORKBENCH_ETAG | ACTOR_SCOPE | 200/304 | VALIDATION_FAILED,UNAUTHENTICATED,NOT_AUTHORIZED,NOT_FOUND,RATE_LIMITED,INTERNAL_ERROR,SERVICE_UNAVAILABLE |
...
| getCommandReceipt | GET | /api/v1/commands/{commandId}/receipt | ACTOR_CONTEXT | NONE | NONE | COMMAND_ID_AND_ACTOR_SCOPE | 200 | VALIDATION_FAILED,UNAUTHENTICATED,NOT_AUTHORIZED,NOT_FOUND,RATE_LIMITED,INTERNAL_ERROR,SERVICE_UNAVAILABLE |
...
TenantSource 的含义是：原业务及管理请求认证完成后由服务端 ActorContext 提供 Tenant；唯一 AUTHENTICATED_IDENTITY 例外为本人 session context，由可信认证的自身 Principal 提供 Tenant，不伪造 Appointment。两者均用于授权、查询和 SQL 绑定；公共调用方不能提交或覆盖 Tenant。内部入口只接受 mTLS worker 身份并映射到受限 ActorContext。

全部32 public Bearer operations 明列可选 X-Appointment-Id，唯一作用是选择服务端给出的本人有效任职。它不改变九份原业务 body DTO，不授予任何权限；SERVICE 不得借该选择器更换静态绑定。Task9.1 仅冻结接口并对齐生成委托签名，实际可信选择与逐请求复验仍属 Task9.2 未完成能力。

## Wire type conventions

除非下表另有说明，所有对象都必须拒绝未声明字段（OpenAPI `additionalProperties: false`）。请求和响应都不得出现 `tenantId`、`commandExecutionSlotId`、SQL 名称、密文、HMAC、授权路径或其他未登记内部标识。ADR-0014 仅允许 Identity 合同具名 DTO/projection 的 Principal/Grant/organization/Appointment selector；该例外不扩大旧业务 DTO，也不允许 caller authority 或 Tenant 输入。`taskId`、`draftId`、`commandId`、`receiptId` 以及 Request DTO 明列的准确领域 selector UUID 均受当前 Actor scope 约束，不能用来读取任意数据库行或显示原始 UUID 文案。未明列主键不得投影，Receipt 结果使用不透明 `factRef`。
