...
The durable journal permits exactly 15 ordered management UI writes: three HUMAN principals, OWNED_ROOT plus EMPTY_ROOT, three appointments, four sourceOwner grants, and three supervisor grants. Existing IdP users are only queried; no IdP administration occurs. Dynamic login verifies sourceOwner, supervisor, and sales, with sales the sole active CONTACT_OPERATOR. The sourceOwner's actual browser session issues the one controlled synthetic `/api/v1/leads` ingress using `sourceAccountCode=R1_AUTO`; there is no capture page. Sales then uses the real responsibility page to save the CONTACT_LEAD Draft, performs a full reload and value check, and invokes the primary CONNECTED_VALID submit once. Every write is reserved with its observed original command identity, canonical body digest, and exact Actor before network forwarding. An uncertain response leaves PENDING evidence and prevents another write; continuation of an existing journal additionally requires the same explicit operation ID.

The approved Task10.5 sales-authority amendment preserves that original 15-command `MANAGEMENT_COMPLETED` checkpoint, then adds exactly one founder-issued DIRECT `SALES_CONTACT_OWNER` grant for the existing sales appointment scoped to OWNED_ROOT. Its separate `SALES_AUTHORITY_COMPLETED` stage precedes identity verification, capture, Draft, and submit. The closed management count is therefore 16 and the complete bounded sequence is 19 writes; an existing operation continues with only the added grant and the remaining three writes, while a pending grant, old capture at index 15, mismatched operation/environment, or malformed stage prefix fails closed.

Success requires exact original receipt recovery and a fixed `law_app_query` read-only closure over the allowed audit view: one ContactResult, Opportunity, DONE Task, CONFIRMED Draft, Receipt, and Audit plus exactly two Events and two matching Outbox rows. The exclusive closed report contains only run/operation/environment identifiers, management count, golden command/fact, and fixed aggregate counts. Credentials, tokens, contact content, protected rows, raw responses, DOM, and raw errors are never reported. Offline tests and source checks do not constitute a golden pass; Root alone performs the reviewed live run and preserves its external process/audit evidence.
