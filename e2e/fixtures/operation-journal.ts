...
import { check, exact, noLinks, uuid } from './local-environment';
...
interface Journal { identity: RunIdentity; commands: Entry[]; stages: CaseId[] }
...
function validate(data: Journal): void {
  ...
  check(new Set(data.stages).size === data.stages.length && data.stages.every((s, i) => s === CASES[i]));
}
export class OperationJournal {
  private text: string;
  private data: Journal;
  constructor(readonly path: string, identity: RunIdentity, private readonly protect: () => void) {
    protect(); noLinks(dirname(path)); check(!existsSync(path + '.pending'));
  ...
  finishStage(id: CaseId): void { this.requirePrevious(id); this.save({ ...this.data, stages: [...this.data.stages, id] }); }
}
