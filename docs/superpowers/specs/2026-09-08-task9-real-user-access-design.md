...
2026-09-09 用户批准的最小核验澄清：上述“同一完整引导的无变化核验”只指原闭包记录的准确 Tenant、根组织、HUMAN、管理员任职、四项管理 Grant 及原 Slot/Receipt/Audit。按受信 Tenant 下的原始 ID 核对所有原字段、版本、时间和生命周期，不再要求整个租户保持首次记录数量。后来新增的身份、授权、委托、对象规则或命令记录不使原结果失效，也不因核验通过而被认定合法。原始集合缺失／修改、生命周期变化、manifest／原 key／操作者不一致仍拒绝且零写入；首次空目标门、禁止追加授权请求与禁止修补／提权不变。
