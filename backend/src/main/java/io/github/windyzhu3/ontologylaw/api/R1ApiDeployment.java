...
final class R1ApiDeployment {
    ...
    record TenantKeys(String encryption,String phoneHmac,String emailHmac,String sourceHmac,String credentialSubjectHmac) {
    ...
    private R1ApiDeployment(RuntimeDatabase database,ActorContextResolver actors,R1ApiServices services){this.database=database;this.actors=actors;this.services=services;}
    static R1ApiDeployment from(Environment environment) {
        try {
            ...
            if(!"MVP-2026-09-08.1".equals(settings.semanticBaseline())||settings.node()==null||!settings.node().matches("[A-Za-z0-9][A-Za-z0-9_.:-]{0,63}"))throw new IllegalArgumentException();
            ...
            var tenants=new HashSet<UUID>();settings.registrations().forEach(r->tenants.add(r.tenantId()));settings.certificates().forEach(r->tenants.add(r.tenantId()));
            if(settings.tenantKeys()==null||!settings.tenantKeys().keySet().equals(tenants))throw new IllegalArgumentException();
            var encryption=new HashMap<UUID,javax.crypto.SecretKey>();var hmac=new HashMap<UUID,Map<LeadProtection.Purpose,javax.crypto.SecretKey>>();var subjects=new HashMap<UUID,byte[]>();
            var distinctKeys=new HashSet<String>();
            for(var entry:settings.tenantKeys().entrySet()) {
                var keys=entry.getValue();var encoded=List.of(keys.encryption(),keys.phoneHmac(),keys.emailHmac(),keys.sourceHmac(),keys.credentialSubjectHmac());
            ...
            var actors=new ActorContextResolver(database::open,new ExternalSubjectProtection(subjects::get),trusts,registrations,certificates);
            ...
            return new R1ApiDeployment(database,actors,new R1ApiServices(database,sources,protection,bindings,settings.node(),cursor));
