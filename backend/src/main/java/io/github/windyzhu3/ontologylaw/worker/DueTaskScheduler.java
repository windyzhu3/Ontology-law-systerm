...
public final class DueTaskScheduler implements AutoCloseable {
    ...
    private static final class State {final ReentrantLock lock=new ReentrantLock();String cursor;int failures;Instant retryAt=Instant.MIN;volatile boolean frozen;}
    ...
    public synchronized void start(){if(closed||scheduler!=null)throw new IllegalStateException("R1_WORKER_ALREADY_STARTED");scheduler=Executors.newScheduledThreadPool(states.size(),Thread.ofPlatform().daemon(true).factory());for(var key:states.keySet())scheduler.scheduleWithFixedDelay(()->poll(key.binding(),key.type()),0,1,TimeUnit.SECONDS);}
