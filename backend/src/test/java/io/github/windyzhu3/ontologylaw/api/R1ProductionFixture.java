...
public abstract class R1ProductionFixture extends R1HttpFixture {
    ...
    protected Deployment deployment(Path directory)throws Exception {
        ...
        var directorySecret=directory.resolve("online-directory.secret");Files.writeString(directorySecret,identityProvider.directorySecret);api.put(human+"directory-client-id","task92-directory");api.put(human+"directory-secret-path",directorySecret.toString());
        api.put("ols.api.identity-administration.active-candidate-key-id","online-v1");api.put("ols.api.identity-administration.candidate-keys[online-v1]",encoded(0x73));api.put("ols.api.identity-administration.etag-key",encoded(0x74));api.put("ols.api.identity-administration.cursor-key",encoded(0x75));
