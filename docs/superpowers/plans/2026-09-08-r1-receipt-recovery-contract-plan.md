...
- [ ] **Step 1: Write the meaningful RED activation test first.** Use a missing-validator diagnostic like the existing readiness tests, not an import error. Start with:
...
- [ ] **Step 3: Add semantic registry and integration mutation tests.** Follow existing `copy_valid_repository`/temporary-copy pattern with valid-control assertions before each mutation, non-vacuous changed-value assertion, and restored-control acceptance. Derive an exact allowed registry independently in validator; reject duplicate/unknown/missing rows and fenced/commented decoys. Keep parsed OpenAPI comparison structural rather than formatting-sensitive. Required mutation families:
