...
状态：**DRAFT — 用户已批准扩大 Task9 范围，并确认 Keycloak/OIDC 方向；本文详细安全、交互和接口设计待书面确认。** 本轮只设计和修订计划，不部署身份服务、不建立实际用户、不授予权限、不改变活动合同。
...
2. 管理员在同 SPA 的 ADM-01，从服务端按本 Tenant realm 只读查询的受控 IdP 账号候选中选择准确账号，创建本地 Principal；然后通过 ADM-02/03/04 建组织、任职、直接授权。候选列表最多 50 条、显式搜索、仅获权管理员可读，不在浏览器持有 IdP 管理凭据，不允许手填任意 subject 冒充绑定。
...
本设计已覆盖用户三项要求，Keycloak 方向已确认；详细接口 inventory、会话恢复标记、管理权限及 bootstrap 例外仍需审阅本文后确认。确认前只能维护设计与计划，不能激活后继合同或开始生产代码。下一步按配套计划的 Task9.1 先收口合同，不跳过安全边界先画表单或开接口。
