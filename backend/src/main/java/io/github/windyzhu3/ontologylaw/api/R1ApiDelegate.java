...
public class R1ApiDelegate implements CommandReceiptsApi,LeadsApi,TaskCommandsApi,TaskDraftsApi,WorkbenchApi {
    ...
    public ResponseEntity<CommandReceipt> getCommandReceipt(UUID commandId){
    ...
    public ResponseEntity<LeadCommandReceipt> captureLead(UUID key,CaptureLeadV1 body){
    ...
    public ResponseEntity<CurrentWorkCardEnvelope> getCurrentWorkCard(String ifNoneMatch){
    ...
    public ResponseEntity<ActionDraftWriteResult> saveActionDraft(UUID task,UUID key,SaveActionDraftV1 body,String match,String none){
    ...
    public ResponseEntity<DecisionRecordCommandReceipt> acknowledgeSourceIntakeStopRequest(UUID task,UUID key,String match,AcknowledgeSourceIntakeStopRequestV1 body){return primary(CommandEnvelope.Type.ACKNOWLEDGE_SOURCE_INTAKE_STOP_REQUEST,task,key,match,body,DecisionRecordCommandReceipt.class);}
    public ResponseEntity<LeadAssignmentCommandReceipt> assignLead(UUID task,UUID key,String match,AssignLeadV1 body){return primary(CommandEnvelope.Type.ASSIGN_LEAD,task,key,match,body,LeadAssignmentCommandReceipt.class);}
    public ResponseEntity<LeadCommandReceipt> completeLeadIngress(UUID task,UUID key,String match,CompleteLeadIngressV1 body){return primary(CommandEnvelope.Type.COMPLETE_LEAD_INGRESS,task,key,match,body,LeadCommandReceipt.class);}
    public ResponseEntity<LeadContactResultCommandReceipt> recordContactResult(UUID task,UUID key,String match,RecordContactResultV1 body){return primary(CommandEnvelope.Type.RECORD_CONTACT_RESULT,task,key,match,body,LeadContactResultCommandReceipt.class);}
    public ResponseEntity<DecisionRecordCommandReceipt> recordRoutingDisposition(UUID task,UUID key,String match,RecordRoutingDispositionV1 body){return primary(CommandEnvelope.Type.RECORD_ROUTING_DISPOSITION,task,key,match,body,DecisionRecordCommandReceipt.class);}
    public ResponseEntity<DecisionRecordCommandReceipt> resolveDuplicateLead(UUID task,UUID key,String match,ResolveDuplicateLeadV1 body){return primary(CommandEnvelope.Type.RESOLVE_DUPLICATE_LEAD,task,key,match,body,DecisionRecordCommandReceipt.class);}
    public ResponseEntity<DecisionRecordCommandReceipt> reviewLeadValidity(UUID task,UUID key,String match,ReviewLeadValidityV1 body){return primary(CommandEnvelope.Type.REVIEW_LEAD_VALIDITY,task,key,match,body,DecisionRecordCommandReceipt.class);}
