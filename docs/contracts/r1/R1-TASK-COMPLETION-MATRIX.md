...
| reopenDueContactTasks | `responsibility.task_occurrence@WAITING→OPEN CAS后的revision` | 同一准确WaitReceipt已经导致该Task为OPEN时的`responsibility.task_occurrence@锁内当前revision` | `R1_REOPEN_SCOPE_V1(tenantId,commandType,taskId,waitReceiptId,waitReceiptHash)` |
| reopenDueRoutingReviewTasks | `responsibility.task_occurrence@WAITING→OPEN CAS后的revision` | 同一准确WaitReceipt已经导致该Task为OPEN时的`responsibility.task_occurrence@锁内当前revision` | `R1_REOPEN_SCOPE_V1(tenantId,commandType,taskId,waitReceiptId,waitReceiptHash)` |
...
`reopenDueRoutingReviewTasks`只处理`RESOLVE_LEAD_ROUTING_GAP`及最新`R1_ROUTING_REVIEW_WAIT_V1` WaitReceipt；contact恢复仍只处理`CONTACT_LEAD`及`CONTACT_RETRY_V1`。两者为静态具名、单Task命令，scope中的`commandType`禁止跨命令复用同一幂等scope。所有Revision、scope和Draft规范化拒绝非JSON安全整数。当前revision为`9007199254740991`且需要递增时，在任何写入前以`INTERNAL_ERROR`失败且不留下slot或Receipt。

两个internal recovery command遵循固定顺序：认证/授权并构造canonical scope；若同key已有slot，则同scope同payload优先重放原Receipt（即使Task后来已越过OPEN），异scope或异payload优先返回`COMMAND_PAYLOAD_CONFLICT`及原Receipt引用；只有新key才进入pre-insert eligibility gate。该gate在`LEAD→TASK→COMMAND advisory lock`保护下验证Task类型、WAITING状态、expected revision、最新WaitReceipt的ID/hash/profile以及due条件，是对通用“业务锁前pre-slot”的具名内部恢复修订，而非无锁状态判断。新key的before-due、wrong task/wait type和stale selector均为`VALIDATION_FAILED`且全部持久化delta为0（包括slot、Receipt和Audit）；只有完整selector通过后才插入slot并进入CAS。
...
| E2E_P0_04_ROUTING_REOPEN | P0_04_SCHEDULE_ROUTING_REVIEW | `wait_receipt:+0` | `routing task:WAITING,r1→OPEN,r2` | `CurrentCard:RESOLVE_LEAD_ROUTING_GAP,r2; next disposition accepted` | `receipt:+1,event:+1,outbox:+1,audit:+1` | `before-due:all-0; wrong-type:all-0; stale-selector:all-0; replay:all-0` |
