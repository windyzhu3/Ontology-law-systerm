...
> 版本补充：v2.2（2026-08-27）。本文件末尾P0一致性补充优先于更早的示例与状态描述。
...
本节是销售主链工作卡的当前权威验收合同。普通用户入口始终保持“一句摘要＋一张已展开且可直接执行的当前WorkCard＋最多两条后续摘要＋一个等待计数＋固定Chat输入框”。TaskOccurrence初始状态只允许OPEN；等待由Query Facade依据未完成责任、外部动作、审批槽和时间门槛投影，不把已完成Task伪造成WAITING。
