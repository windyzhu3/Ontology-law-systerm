...
Task9 identity authority: [ADR-0014](../../../docs/adr/ADR-0014-task9-real-user-access.md); profile: R1_IDENTITY_ACCESS_V1

The named Task9 successor [Identity contract](../../../docs/contracts/r1/R1-IDENTITY-ACCESS-CONTRACT.md) freezes R1_IDENTITY_COMMAND_V1, R1_IDENTITY_DISCLOSURE_V1, R1_IDENTITY_SELF_DISCLOSURE_V1 and R1_IDENTITY_BOOTSTRAP_V1. Online Identity writers take R1_BUSINESS_TENANT_LOCK exclusive → command UUID fence → identity Tenant exclusive → ordered Identity rows; no later Lead/Task lock. Responsibility dependencies use the Owner narrow read port. Existing business locks/events/Worker authority are unchanged. Self-query is a verified own HUMAN Principal with physically nullable Audit Appointment, not a fabricated business Actor. Offline SYSTEM is authorization origin only, with exact new-founder/operator attribution and closed initial delta. V010/V020/V040/V830 already support these named semantics and exact Identity writes; no new GRANT, membership, application table or migration-byte change. Keycloak separately owns external storage outside business13 schemas/52+2 at52-plus-2-v1.2. These profiles qualify prior general four-axis and Audit authorization-Fact wording only as explicitly specified; all other rules remain.

Receipt recovery authority: [ADR-0013](../../../docs/adr/ADR-0013-r1-command-receipt-recovery.md); profile: R1_RECEIPT_RECOVERY_METADATA_LOOKUP_V1

2026-09-08 named semantic supersession only: ADR-0013 §§4–6 qualifies the general current four-axis/audit-query self-audit and high-risk refusal rules below, including their V830 comment formulation. Only API receipt service may call Audit Owner's QUERY narrow port on `audit.audit_entry_classified_v` before full business authorization and without recursive query Audit. Valid uniquely bound Bearer Actor, fresh active identity/organization, Tenant business shared lock then identity shared lock, READ COMMITTED, exact trusted Tenant/commandId/original Principal/Appointment/NULL-safe on-behalf predicate, original EVENT command rows and limit 2 are mandatory. Return only matching fixed metadata, schema/version, Subject/Scope, command type/result, receiptRecovery and digest verification inputs; validate whole-summary integrity inside Audit Owner and never return change_summary or authorizationEvidence. No base-table read, GRANT expansion, actorless fallback, browsing, Worker or internal recovery use is allowed.

Only this non-disclosing GET profile has zero refusal delta; it never exempts general high-risk refusals. Current Owner authorization is fully recomputed before switching to AUDIT, appending READ_COMMAND_RECEIPT and confirming the same transaction commit before serialization. All primary and extra Subject decisions bind authorization_snapshot_digest; authorization_fact records one complete primary path. Audit/lock failures roll back uncommitted work; commit acknowledgement loss may already have committed Audit and never permits sensitive disclosure. V830 source, generated copy, render and manifest bytes are unchanged; no physical or permission exception is created. Original Task8 must implement and verify this protocol.
