...
class R1ProjectionDispatcherTest {
    @Test void unsafe_origin_and_unregistered_credentials_never_construct_a_transport(){
        var binding=new R1WorkerTenantBindings.Binding(UUID.randomUUID(),UUID.randomUUID(),UUID.randomUUID(),"CLIENT","a".repeat(64));var registry=new R1WorkerTenantBindings("MVP-2026-09-07.1",List.of(binding));
    ...
    @Test void registry_snapshot_is_immutable_and_duplicate_aliases_cannot_select_a_different_key(){
        var binding=new R1WorkerTenantBindings.Binding(UUID.randomUUID(),UUID.randomUUID(),UUID.randomUUID(),"CLIENT","a".repeat(64));var mutable=new ArrayList<>(List.of(binding));var registry=new R1WorkerTenantBindings("MVP-2026-09-07.1",mutable);mutable.clear();assertEquals(List.of(binding),registry.bindings());assertThrows(UnsupportedOperationException.class,()->registry.bindings().clear());
        var second=new R1WorkerTenantBindings.Binding(UUID.randomUUID(),UUID.randomUUID(),UUID.randomUUID(),"client","b".repeat(64));assertThrows(IllegalArgumentException.class,()->new R1WorkerTenantBindings("MVP-2026-09-07.1",List.of(binding,second)));
    }
}
