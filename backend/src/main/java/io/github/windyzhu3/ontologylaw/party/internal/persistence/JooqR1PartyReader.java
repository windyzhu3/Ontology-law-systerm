...
public final class JooqR1PartyReader implements R1PartyReader {
    public NamedParty named(Connection c,UUID tenant,UUID id) {
        var p=PARTY_;var r=DSL.using(c,SQLDialect.POSTGRES,new org.jooq.conf.Settings().withExecuteLogging(false))
                .select(p.REVISION,p.CANONICAL_NAME,p.STATUS).from(p).where(p.TENANT_ID.eq(tenant)).and(p.PARTY_ID.eq(id)).fetchOne();
        return r==null?null:new NamedParty(id,r.value1(),r.value2(),r.value3());
    }
