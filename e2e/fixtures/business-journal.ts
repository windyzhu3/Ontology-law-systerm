...
export type BusinessCaseId = typeof BUSINESS_CASES[number] | typeof CONTACT_WAIT_CASE;
...
const PROFILES = Object.freeze({
  business: Object.freeze({ steps: Object.freeze([...BUSINESS_STEPS]), cases: Object.freeze([...BUSINESS_CASES]) as readonly BusinessCaseId[], ends: Object.freeze([...STAGE_END]) as readonly number[], prefix: 'task9-business' }),
  contactWait: Object.freeze({ steps: Object.freeze(['capture-manual', 'assign-draft', 'assign-submit', 'contact-draft', 'contact-submit']) as readonly BusinessStep[], cases: Object.freeze([CONTACT_WAIT_CASE]) as readonly BusinessCaseId[], ends: Object.freeze([5]), prefix: 'task9-contact-wait' }),
});
...
function validCommand(command: BusinessCommand, index?: number, profile: Profile = PROFILES.business): void {
  check(exact(command, ['step', 'commandId', 'method', 'path', 'bodySha256', 'actorScopeKey', 'requestSelectors']));
  check(profile.steps.includes(command.step) && (index === undefined || command.step === profile.steps[index]));
...
function validate(data: Journal, profile: Profile = PROFILES.business): void {
  check(exact(data, ['identity', 'commands', 'stages'])); validIdentity(data.identity);
  check(Array.isArray(data.commands) && data.commands.length <= profile.steps.length && Array.isArray(data.stages));
  const keys = new Set<string>();
  for (const [index, entry] of data.commands.entries()) {
    const { at, status, httpStatus, receiptId, resultFact, selectors, ...command } = entry; validCommand(command, index, profile);
  ...
  check(data.stages.length <= profile.cases.length && data.stages.every((stage, index) => exact(stage, ['caseIdentity', 'reportPath', 'reportSha256', 'status', 'exitCode']) && stage.caseIdentity === profile.cases[index] && HASH.test(stage.reportSha256) && stage.status === 'PASSED_SUBSCENARIO' && stage.exitCode === 0 && (profile === PROFILES.business || data.commands.length >= profile.ends[index]) && data.commands.slice(0, profile.ends[index]).every(x => x.status === 'CONFIRMED')));
}
...
export class BusinessJournal {
  ...
  private constructor(readonly path: string, private readonly guard: () => void | Promise<void>, private readonly restart?: BusinessRestart, private readonly profile: Profile = PROFILES.business) {}
  ...
  static async open(path: string, identity: BusinessRunIdentity, protect: () => void | Promise<void>, restart?: BusinessRestart): Promise<BusinessJournal> {
    check(basename(path) !== 'task9-contact-wait-operation.json');
  ...
  static async openContactWait(path: string, identity: BusinessRunIdentity, protect: () => void | Promise<void>): Promise<BusinessJournal> {
    check(basename(path) === 'task9-contact-wait-operation.json');
    check(identity.predecessorRunId === '9848f4ee-5612-49df-9e10-a8c40c09bd3d' && identity.predecessorSha256 === '841a4d275bf97bdb832bbb138f70efbc310dbbe10f532d6c4dbb435380d52333' && identity.runId !== identity.predecessorRunId);
    const journal = new BusinessJournal(path, protect, undefined, PROFILES.contactWait); await journal.initialize(structuredClone(identity)); return journal;
  }
  private async initialize(identity: BusinessRunIdentity) {
    ...
    if (existsSync(this.path)) { noLinks(this.path); this.text = readFileSync(this.path, 'utf8'); this.data = JSON.parse(this.text); validate(this.data, this.profile);
    ...
    else { check(!this.restart); this.data = { identity, commands: [], stages: [] }; validate(this.data, this.profile); this.text = JSON.stringify(this.data); durableExclusive(this.path, this.text); await this.protect(); }
    this.verifyEvidence();
  }
  ...
  stageCommands(id: BusinessCaseId): BusinessEntry[] { const index = this.profile.cases.indexOf(id); check(index >= 0); return structuredClone(this.data.commands.slice(index ? this.profile.ends[index - 1] : 0, this.profile.ends[index])); }
  ...
  private validateEvidencePath(path: string, id: BusinessCaseId) { const prefix = `${this.profile.prefix}-${this.data.identity.runId}-${id}-`; check(dirname(path) === dirname(this.path) && basename(path).startsWith(prefix) && basename(path).endsWith('.json') && uuid.test(basename(path).slice(prefix.length, -5))); }
  ...
  private async save(next: Journal) {
    this.requireClear(); this.verifyEvidence(); await this.protect(); this.requireClear(); noLinks(this.path); validate(next, this.profile); this.restart?.assertJournal(this.path, this.restart.activeIdentity, next);
  ...
  async begin(command: BusinessCommand): Promise<void> { const original = structuredClone(command); return this.mutate(async () => {
    validCommand(original, this.data.commands.length, this.profile); check(!this.pending() && this.data.commands.every(x => x.status === 'CONFIRMED'));
    const completedBoundaries = this.profile.ends.filter(end => end <= this.data.commands.length).length; check(this.data.stages.length === completedBoundaries);
  ...
  private requirePreviousState(id: BusinessCaseId) { this.requireClear(); this.verifyEvidence(); check(!this.pending()); const index = this.profile.cases.indexOf(id), start = index > 0 ? this.profile.ends[index - 1] : 0; check(index >= 0 && this.data.stages.length === index && this.data.commands.length >= start && this.data.commands.length <= this.profile.ends[index] && this.data.commands.every(x => x.status === 'CONFIRMED')); }
  async finishStage(id: BusinessCaseId, evidence: BusinessEvidence, io: CompletionIO = {}): Promise<void> { evidence = structuredClone(evidence); return this.mutate(async () => {
    const index = this.profile.cases.indexOf(id); check(index >= 0 && this.data.stages.length === index && this.data.commands.length === this.profile.ends[index] && this.data.commands.every(x => x.status === 'CONFIRMED'));
    ...
    validate(next, this.profile); const nextBytes = JSON.stringify(next), intent = this.path + '.completion.pending'; durableExclusive(intent, nextBytes);
