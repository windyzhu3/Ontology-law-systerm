...
public final class JooqAuditAppender implements AuditAppender {
    ...
    public void append(Connection connection,Entry e)throws SQLException {
        write(connection,e.id(),e.commandId(),e.commandType(),e.correlationId(),e.commandType(),e.result(),e.authorization(),e.authorization().request().subject(),"R1_COMMAND_AUDIT_V1",e.summary(),e.summaryDigest());
    }
    public void append(Connection connection,ReadDisclosureEntry e)throws SQLException {
        write(connection,e.id(),null,null,e.correlationId(),"READ_CURRENT_WORKCARD","SUCCEEDED",e.authorization(),e.disclosedSource(),"R1_CURRENT_WORKCARD_DISCLOSURE_AUDIT_V1",e.summary(),e.summaryDigest());
    }
    private void write(Connection connection,java.util.UUID id,java.util.UUID commandId,String commandType,java.util.UUID correlation,
            ...
            io.github.windyzhu3.ontologylaw.identity.AuthorizationService.Subject subject,String schema,String summary,byte[] summaryDigest) {
        ...
        DSL.using(connection,SQLDialect.POSTGRES,new org.jooq.conf.Settings().withExecuteLogging(false))
                ...
                .set(a.SUMMARY_SCHEMA_CODE,schema).set(a.SUMMARY_SCHEMA_VERSION,1).set(a.CHANGE_SUMMARY,JSONB.valueOf(summary)).set(a.CHANGE_SUMMARY_DIGEST,summaryDigest)
