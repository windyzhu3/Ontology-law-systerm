...
public final class JooqIdentityBootstrapService implements IdentityBootstrapService {
    ...
    public void verify(Connection c,Manifest m,byte[] hmac,Facts f)throws SQLException {
        ...
        var p=PRINCIPAL;exact(d,p,p.TENANT_ID.eq(tenant),p.PRINCIPAL_ID.eq(f.principal()).and(p.PRINCIPAL_KIND.eq("HUMAN")).and(p.IDENTITY_PROVIDER_CODE.eq(m.identityProviderCode())).and(p.EXTERNAL_SUBJECT_HMAC.eq(hmac)).and(p.DISPLAY_NAME.eq(m.principalDisplayName())).and(p.STATE.eq("ACTIVE")).and(p.REVISION.eq(0L)).and(p.CREATED_AT.eq(now)),1);
        var o=ORGANIZATION_UNIT;exact(d,o,o.TENANT_ID.eq(tenant),o.ORGANIZATION_UNIT_ID.eq(f.root()).and(o.PARENT_ORGANIZATION_UNIT_ID.isNull()).and(o.UNIT_CODE.eq(m.rootCode())).and(o.DISPLAY_NAME.eq(m.rootDisplayName())).and(o.STATE.eq("ACTIVE")).and(o.CLOSED_AT.isNull()).and(o.REVISION.eq(0L)).and(o.CREATED_AT.eq(now)),1);
        var a=APPOINTMENT;exact(d,a,a.TENANT_ID.eq(tenant),a.APPOINTMENT_ID.eq(f.appointment()).and(a.PRINCIPAL_ID.eq(f.principal())).and(a.ORGANIZATION_UNIT_ID.eq(f.root())).and(a.ROLE_CODE.eq("IDENTITY_ADMIN")).and(a.EFFECTIVE_FROM.eq(effective)).and(a.EFFECTIVE_UNTIL.isNull()).and(a.STATE.eq("ACTIVE")).and(a.REVISION.eq(0L)).and(a.CREATED_AT.eq(now)),1);
        var g=AUTHORITY_GRANT;exact(d,g,g.TENANT_ID.eq(tenant),g.AUTHORITY_GRANT_ID.in(f.grants()).and(g.GRANTEE_APPOINTMENT_ID.eq(f.appointment())).and(g.GRANTED_BY_APPOINTMENT_ID.eq(f.appointment())).and(g.SCOPE_ORGANIZATION_UNIT_ID.eq(f.root())).and(g.AUTHORITY_CODE.in(MANAGEMENT_CODES)).and(g.VALID_FROM.eq(effective)).and(g.VALID_UNTIL.isNull()).and(g.STATE.eq("ACTIVE")).and(g.REVOKED_AT.isNull()).and(g.REVOCATION_REASON_CODE.isNull()).and(g.REVISION.eq(0L)).and(g.CREATED_AT.eq(now)),4);
        if(d.selectCount().from(g).where(g.TENANT_ID.eq(tenant)).groupBy(g.AUTHORITY_CODE).fetch().size()!=4)throw invalid();
        ...
        var dg=DELEGATION_GRANT;exact(d,dg,dg.TENANT_ID.eq(tenant),DSL.trueCondition(),0);var og=OBJECT_ACCESS_GRANT;exact(d,og,og.TENANT_ID.eq(tenant),DSL.trueCondition(),0);
    }
