...
## Task 10.3: controlled R1 identity bootstrap consumer

`e2e/runtime/r1_bootstrap.py` accepts only a current `INFRASTRUCTURE_READY` run created under this worktree. The controller-only `bootstrap <run>` operation revalidates the protected environment, exact Compose inventory, pinned Java, current Jar/schema deployment state, and then invokes the existing `IdentityBootstrapCommand` in the fixed candidate, dry-run, confirmed execute, and original-verify sequence for `R1_E2E_MAIN` and `R1_E2E_ISOLATION`. A protected, digest-bound copy of `e2e/runtime/r1-bootstrap-logback.xml` routes offline framework logs to stderr so stdout remains one strict JSON document; configuration drift blocks original verification. It never starts API, Worker, or SPA and its terminal state keeps `applicationReady=false`.
...
## Task 10.5: isolated R1 lead-to-contact golden consumer

`e2e/r1-isolated.config.ts` defaults to the offline harness. The real project is discoverable only through an exact `--project approved-r1-isolated` selection and requires `R1_ISOLATED_ACCEPTANCE=APPROVED_SYNTHETIC_ONLY`, the current protected `R1_E2E_RUN`, and a fresh `R1_ISOLATED_OPERATION_ID` UUID. It pins one worker, zero retries, the prepared Node/Playwright/Chromium toolchain, strict TLS, and no recording, storage state, debug mode, or reporter override. Browser trust is a controller-owned temporary CurrentUser prerequisite for the exact run CA; this source neither changes trust nor bypasses TLS.

The durable journal permits exactly 15 ordered management UI writes: three HUMAN principals, OWNED_ROOT plus EMPTY_ROOT, three appointments, four sourceOwner grants, and three supervisor grants. Existing IdP users are only queried; no IdP administration occurs. Dynamic login verifies sourceOwner, supervisor, and sales, with sales the sole active CONTACT_OPERATOR. The sourceOwner's actual browser session issues the one controlled synthetic `/api/v1/leads` ingress using `sourceAccountCode=R1_AUTO`; there is no capture page. The approved supplemented browser sequence after capture is described below. Every write is reserved with its observed original command identity, canonical body digest, and exact Actor before network forwarding. An uncertain response leaves PENDING evidence and prevents another write; continuation of an existing journal additionally requires the same explicit operation ID.

The approved Task10.5 sales-authority amendment preserves that original 15-command `MANAGEMENT_COMPLETED` checkpoint, then adds exactly one founder-issued DIRECT `SALES_CONTACT_OWNER` grant for the existing sales appointment scoped to OWNED_ROOT. Its separate `SALES_AUTHORITY_COMPLETED` stage precedes identity verification and capture. Because the approved capture intentionally contains no contact value, the sourceOwner next uses the real page to save, fully reload, and submit the existing `COMPLETE_LEAD_INGRESS` Draft with reserved synthetic contact data. The resulting automatic assignment is bound through a narrow `law_app_query` read-only projection to the original capture Lead before sales adopts the successor `CONTACT_LEAD` card. Sales then saves, reloads, and submits the contact Draft. The closed management count remains 16 and the complete bounded sequence is 21 writes; the retained 17-command capture checkpoint continues with exactly the four remaining writes. `INGRESS_DRAFT_RELOADED` and `INGRESS_COMPLETED` are durable stages, and old contact writes at index 17, pending writes, mismatched operation/environment, or malformed stage prefixes fail closed.

Success requires exact original receipt recovery and a fixed `law_app_query` read-only closure over the allowed audit view: the original capture receipt, exact ingress Task/Draft/receipt, automatic Assignment, same-Lead successor contact Task, and the existing ContactResult/Opportunity/DONE Task/CONFIRMED Draft/Receipt/Audit plus exactly two Events and two matching Outbox rows. Database closure validates internal Subject types; browser receipts retain the public DTO fact enums. The exclusive closed report uses `CAPTURE_INGRESS_AUTOASSIGN_CONTACT_V1` so this supplemented flow cannot be mistaken for direct-capture automatic assignment, and otherwise contains only run/operation/environment identifiers, management count, golden command/fact, and fixed aggregate counts. Credentials, tokens, contact content, protected rows, raw responses, DOM, and raw errors are never reported. Offline tests and source checks do not constitute a golden pass; Root alone performs the reviewed live run and preserves its external process/audit evidence.
