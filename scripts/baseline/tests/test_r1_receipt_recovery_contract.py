...
class R1ReceiptRecoveryContractTest(unittest.TestCase):
    ...
    def test_active_versions_and_resolved_authority_pointers_are_required(self):
        for relative, old in ((BASELINE, 'Baseline ID: MVP-2026-09-08.1'),
                              (HTTP, 'Contract ID: R1-HTTP-V1.3'),
                              (COMMAND, 'Contract ID: R1-COMMAND-POLICY-EVENT-V1.2')):
            for new in ('UNKNOWN_ACTIVE_VERSION', old.replace('09-08.1', '09-07.1').replace('V1.3', 'V1.2').replace('EVENT-V1.2', 'EVENT-V1.1')):
